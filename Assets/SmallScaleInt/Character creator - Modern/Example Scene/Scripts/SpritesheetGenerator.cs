...
namespace SmallScaleInc.CharacterCreatorModern
{
    public class SpritesheetGenerator : MonoBehaviour
    {
        ...
        "TakeDamage", "Taunt"
            ...
            //탈 것 없으면 렌더 안함 -> 탈 것 없으면 탈 것 빼고 렌더
            if (!string.IsNullOrEmpty(mountNameText.text) && mountNameText.text != "None")
            //if (!string.IsNullOrEmpty(mountNameText.text))
            {
                        ...
                        if (tex != null)
                        {
                            ...
                            for (int y = 0; y < rows; y++)
                            {
                                for (int x = 0; x < columns; x++)
                                {
                                    ...
                                    smd.name = Path.GetFileNameWithoutExtension(assetPath) + "_" + y + "_" + x + "_" + spriteIndex++; ;
